...
def tent(x: NDArray[np.floating], params: TentParams) -> NDArray[np.floating]:
    ...
    return out
    # TODO: maybe reword neurons here to reflect they're  sensors 

def compute_activations(neurons: NDArray[np.floating], input_x: NDArray[np.floating], config: RegressionParams) -> NDArray[np.floating]:
    ...
    if not  n_dims_neuron == n_dims_input:
       raise ValueError(f"Mismatch between neuron dimension [axis 0]={n_dims_neuron} and input dimension [axis=0]={n_dims_input}")
        
    # input dimensions must match that contained in the config 
    if not  n_dims_neuron == config.input_dimension:
       raise ValueError(f"Mismatch between neuron dimension [axis 0]={n_dims_neuron} and input dimension set in config={n_dims_input}")
    
    # number of neurons (width) must match that contained in the config
    if not  width == config.width:
       raise ValueError(f"Mismatch between number of neurons provided={width} and the number of neurons set in config={config.width}")
    ...
    if not transform:
        raise RuntimeError(f"Transform '{config.transform_type.name}' could not be found.")
