...
def gram_schmidt_loop(u: np.ufunc, transform: np.ufunc) -> np.ufunc:
    ...
    try:
        ...
        raise RuntimeError(
            f"The provided function {u} does not appear to be a numpy universal function:{traceback.format_exc()}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"The provided transform {transform} does not appear to be a numpy universal function:{traceback.format_exc()}"
        )
